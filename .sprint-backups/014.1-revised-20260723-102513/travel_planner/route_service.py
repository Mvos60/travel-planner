...
# SPRINT 014.1: OSRM HTTP response cache
#
# The cache wraps the existing low-level HTTP JSON method instead of changing
# provider route parsing. This keeps the provider's established behaviour
# intact while preventing identical OSRM requests from reaching the network.
from dataclasses import asdict as _route_cache_asdict, is_dataclass as _route_cache_is_dataclass
from enum import Enum as _RouteCacheEnum
import hashlib as _route_cache_hashlib
import json as _route_cache_json

from travel_planner.route_cache import RouteCache as _RouteCache


_ROUTE_CACHE_HTTP_METHOD_NAME = '_build_json_request'
_travel_planner_uncached_http_method = getattr(
    BaseHttpRouteProvider,
    _ROUTE_CACHE_HTTP_METHOD_NAME,
)


def _route_cache_normalize(value):
    if value is None or isinstance(value, (str, int, float, bool)):
        return value

    if isinstance(value, _RouteCacheEnum):
        return _route_cache_normalize(value.value)

    if _route_cache_is_dataclass(value):
        return _route_cache_normalize(_route_cache_asdict(value))

    if isinstance(value, dict):
        return {
            str(key): _route_cache_normalize(item)
            for key, item in sorted(value.items(), key=lambda pair: str(pair[0]))
        }

    if isinstance(value, (list, tuple)):
        return [_route_cache_normalize(item) for item in value]

    if isinstance(value, set):
        return sorted(
            (_route_cache_normalize(item) for item in value),
            key=repr,
        )
    ...
    if hasattr(value, "__dict__"):
        return {
            ...
            "attributes": _route_cache_normalize(vars(value)),
        }

    return repr(value)


def _build_osrm_http_cache_key(args, kwargs):
    request_data = {
        "provider": "osrm",
        "method": _ROUTE_CACHE_HTTP_METHOD_NAME,
        "args": _route_cache_normalize(args),
        "kwargs": _route_cache_normalize(kwargs),
    }
    encoded = _route_cache_json.dumps(
        request_data,
        ensure_ascii=False,
        sort_keys=True,
        separators=(",", ":"),
    ).encode("utf-8")
    return _route_cache_hashlib.sha256(encoded).hexdigest()


def _cached_http_method(self, *args, **kwargs):
    if self.__class__.__name__ != "OSRMRouteProvider":
        return _travel_planner_uncached_http_method(self, *args, **kwargs)
    ...
    key = _build_osrm_http_cache_key(args, kwargs)
    cached = cache.get(key)
    if cached is not None and "response" in cached:
        return cached["response"]

    response = _travel_planner_uncached_http_method(self, *args, **kwargs)
